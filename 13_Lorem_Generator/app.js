...
form.addEventListener('submit', (e) => {
  //form submit default behavior is sending data to server, so use preventDefault(); before applying our own script
  e.preventDefault();
  //any <input> will return a string, use parseInt() to pass the value
  //remember : black is string violet is
  const value = parseInt(amount.value);
  const randomText = Math.floor(Math.random() * text.length)
  //limit value (empty, -1, >9) in conditional statement
